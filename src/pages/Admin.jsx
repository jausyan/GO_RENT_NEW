...
function LoginForm({ onLogin }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-2">
      <div className="w-full max-w-sm bg-white rounded-3xl border-2 border-gray-200 p-7 space-y-5 shadow-sm">
        <div className="text-center">
          <h1 className="text-xl font-extrabold text-gray-800">Admin Panel</h1>
          <p className="text-gray-400 text-sm mt-0.5">GO-RENT</p>
        </div>
        <form onSubmit={submit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-extrabold py-4 rounded-2xl text-base transition-all disabled:opacity-60">
            {loading ? 'Masuk...' : 'Masuk'}
          </button>
        </form>
      </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <p className="font-extrabold text-gray-800 tracking-widest text-base">{order.order_code}</p>
            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${st.badge}`}>
              {st.label}
            </span>
          </div>
          <button onClick={onClose}
            className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-base active:scale-95 transition-all">
            X
          </button>
        </div>

        {/* info rows */}
        <div className="px-5 py-4 space-y-0 text-sm divide-y divide-gray-50">
          {[
        ...
        {lines.length > 0 && (
          <div className="px-5 pb-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-2">Item</p>
            <div className="space-y-2">
              {lines.map((l) => (
                <div key={l.id} className="flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2.5">
                  <span className="text-sm text-gray-700">{l.items?.name} × {l.quantity} × {l.rental_days}h</span>
        ...
        {/* total */}
        <div className="mx-5 mb-4 flex justify-between items-center bg-blue-600 rounded-2xl px-4 py-4">
        ...
        <div className="px-5 pb-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-2">Bukti Pembayaran</p>
          {proofUrl ? (
            <a href={proofUrl} target="_blank" rel="noreferrer">
              <img
                src={proofUrl}
                alt="bukti pembayaran"
                className="w-full rounded-2xl border-2 border-gray-200 object-contain max-h-60 active:opacity-80 transition-opacity"
              />
            </a>
          ) : order.payment_proof_url ? (
            <div className="w-full rounded-2xl border-2 border-dashed border-gray-200 h-24 flex items-center justify-center text-gray-400 text-sm">
        ...
        {order.status === 'pending' && (
          <div className="px-5 pb-7 space-y-3 border-t border-gray-100 pt-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">
                Catatan (opsional)
              </label>
              <textarea
                className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:border-blue-400"
                rows={2}
                value={note}
            ...
            {error && (
              <p className="text-sm text-red-600 bg-red-50 rounded-xl px-3 py-2.5 border border-red-200">{error}</p>
            )}
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => update('approved')} disabled={loading}
                className="bg-green-500 hover:bg-green-600 active:scale-95 text-white font-extrabold py-4 rounded-2xl text-sm transition-all disabled:opacity-60">
                Setujui
              </button>
              <button onClick={() => update('rejected')} disabled={loading}
                className="bg-red-500 hover:bg-red-600 active:scale-95 text-white font-extrabold py-4 rounded-2xl text-sm transition-all disabled:opacity-60">
                Tolak
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between pt-2">
        <div>
          <h1 className="text-xl font-extrabold text-gray-800">Admin Panel</h1>
          <p className="text-gray-400 text-xs mt-0.5">Login sebagai <strong>{adminUser}</strong></p>
        </div>
        <button
          onClick={logout}
          className="text-xs font-semibold text-gray-500 border-2 border-gray-200 px-4 py-2 rounded-xl hover:border-red-300 hover:text-red-500 active:scale-95 transition-all"
        >
          Keluar
        </button>
      </div>
      ...
      {loading && <p className="text-center text-gray-400 text-sm py-8">Memuat...</p>}
        ...
        {orders.map((o) => {
          ...
          return (
            <button
              key={o.id}
              onClick={() => openOrder(o)}
              className="w-full text-left bg-white border-2 border-gray-200 rounded-2xl px-4 py-4 hover:border-blue-300 active:scale-[0.98] transition-all shadow-sm"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-extrabold text-gray-800 tracking-wide text-sm">{o.order_code}</p>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${st.badge}`}>
                      {st.label}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1 truncate">{o.customer_name}</p>
                  {o.agency && <p className="text-xs text-gray-400 truncate">{o.agency}</p>}
                  <p className="text-xs text-gray-400 mt-1">{fmtTs(o.created_at)}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="text-sm font-extrabold text-gray-800">{fmt(o.total_price)}</p>
                  <p className="text-xs text-gray-400 mt-1">{o.rental_days} hari</p>
                </div>
              </div>
            </button>
          )
